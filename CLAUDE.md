...
| `offline.html` | 8 | Offline Maps: cuando no hay señal | 9 |
...
flutter_map_tile_caching: ^10.1.1  # FMTC, cache y descarga offline
...
### FMTC para offline maps — API v10.1, verificado en dispositivo real (corregido 2026-07-22)
...
El tile provider del mapa en vivo se construye con `FMTCTileProvider(stores: {'descargas': BrowseStoreStrategy.read}, loadingStrategy: BrowseLoadingStrategy.cacheFirst)` — **`read`, no `readUpdateCreate`**: navegar el mapa con señal no debe cachear nada por su cuenta, solo el botón de descarga explícita llena el store (decisión de diseño deliberada, probada con `readUpdateCreate` automático y revertida por ser menos predecible para el estudiante). `FMTCStore(...).getTileProvider()` sigue existiendo pero está deprecado, y la clase `FMTCTileProviderSettings` ya no existe.  
`CacheBehavior` es un alias deprecado de `BrowseLoadingStrategy` — usar el nombre nuevo en código nuevo.  
`store.download.check(region)` está **deprecado desde v10.1** en favor de `store.download.countTiles(region)` — mismo `Future<int>` (solo cuenta de tiles), nombre más claro. El tamaño en MB no se entrega directo, hay que estimarlo multiplicando por un promedio por tile (~3KB para tiles OSM).  
`store.download.startForeground(region: region)` es sincrónico (no lleva `await`) y devuelve un record `({tileEvents, downloadProgress})` — escuchar con `download.downloadProgress.listen(...)`. El stream se cierra igual si la descarga termina bien o si se cancela — comparar `DownloadProgress.successfulTilesCount` vs `.maxTilesCount` es la única forma de confirmar si de verdad se completó.  
La región a descargar se arma con `mapController.camera.visibleBounds` + zoom actual (rango zoom-1 a zoom+3) — **no un BBox fijo**. `tile.openstreetmap.org` prohíbe expresamente la descarga masiva ("bulk downloading") en su política de uso (`flutter_map` lo advierte al iniciar la app); una región fija de La Paz a zoom 10-16 (~15.000 tiles) fallaba siempre por esto en pruebas reales, no por un bug de código. Mitigación: `parallelThreads: 2` (default: 5), `rateLimit: 10`, y `userAgentPackageName` también en la descarga masiva (no solo en el `TileLayer` del mapa).  
...
- `offline.html` — corregida (2026-07-20): faltaba un `</div>` de cierre en la 2.ª slide que anidaba TODAS las slides siguientes y el navbar dentro de ella — al no ser `.active` quedaban ocultas, por eso "no se veía nada" al navegar. La API de FMTC estaba desactualizada a v10: `FlutterMapTileCaching.initialise()` → `FMTCObjectBoxBackend().initialise()`, `getTileProvider(FMTCTileProviderSettings(...))` (deprecado) → `FMTCTileProvider(stores: {...}, loadingStrategy: ...)`, y `download.check()` ya no devuelve tamaño en MB (solo cuenta tiles — hay que estimar el tamaño multiplicando por un promedio por tile). Se eliminó la slide "Conectividad automatica" (connectivity_plus) — ya se enseña en `collect.html` Clase 5 (ColectaService.escucharConectividad()) y era redundante aquí; en su lugar se agregó un callout sobre `download.pause()`/`resume()`.
  Reescrita a fondo (2026-07-22) para reflejar la implementación real verificada en `carpetanueva/geocollect_app/` (dispositivo Android físico, no solo teoría): (1) la región a descargar ya no es un BBox fijo de La Paz — se arma con `mapController.camera.visibleBounds` + zoom actual, igual que Google Maps u Organic Maps; (2) el store del mapa en vivo usa `BrowseStoreStrategy.read` (no `readUpdateCreate`) — navegar con señal ya NO cachea nada por su cuenta, la única forma de agregar tiles es el botón de descarga, decisión deliberada explicada en su propia slide; (3) nueva slide sobre por qué `tile.openstreetmap.org` prohíbe la descarga masiva en su política de uso (el propio `flutter_map` lo advierte al iniciar la app) y cómo mitigarlo (`parallelThreads: 2`, `rateLimit: 10`, `userAgentPackageName` también en la descarga) — la región fija de 15.000 tiles de la versión anterior fallaba siempre por esto, no por un bug de código; (4) `check()` reemplazado por `countTiles()` (deprecado en FMTC v10.1); (5) nueva slide de verificación: un tile de cache y uno de red se ven idénticos, `FMTCStore.stats` (hits/misses/tiles/size) es la única forma real de comprobar que el mapa usa lo descargado, más el gotcha de que la Clase 2 recentra el mapa con el GPS real y puede sacarte del área descargada al probar modo avión. 9 slides, nav-dots corregidos de 12 a 9 (mismo problema de desincronización que tenía `export.html` antes de su propia corrección).
